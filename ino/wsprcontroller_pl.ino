...
char lastSentBuffer[64]  = "--- brak ---";
char lastSentTime[6]     = "--:--";
unsigned long lastSentId = 0;

//------------------ OSTATNIO ODEBRANY UART ------------------
char lastUartBuffer[128] = "--- brak ---";
char lastUartTime[9]     = "--:--:--";
...
String makeSchedBlock(int idx) {
  ...
  p += "Harmonogram " + String(idx + 1) + "</label></div>";
  ...
  p += "<label style='margin:0;font-size:11px;color:#aaa;'>Godzina:</label>";
  p += "<input type='time' name='sched_time" + si + "' value='" + sendBuf + "'>";
  p += "</div>";

  p += "<div class='sched-freq'>";
  p += "<label style='margin:0;font-size:11px;color:#aaa;'>Cz&#281;stotliwo&#347;&#263;:</label>";
...
void handleRoot() {
  ...
  page += F("'></label><hr>"
            ...
            "<label>Has&#322;o:<input type='password' name='pass' placeholder='password'></label>"
            "<hr><h3>Ustawienia NTP</h3>"
            "<label>Strefa czasowa (offset od UTC w godzinach):"
            "<input type='number' name='tz' value='");
  page += String(timeOffsetHours);
  page += F("' min='-12' max='14' step='0.5'></label>"
            "</div></div></div>"
            "<p style='text-align:center;margin-top:15px;'>"
            "<button type='submit'>Zapisz ustawienia</button></p>"
            "</form></div></body></html>");
